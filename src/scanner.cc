...
struct Scanner {
  ...
  unsigned serialize(char *buffer) {
    ...
    std::memcpy((void *)&(buffer[n_copied_so_far]),
                (void *)&queued_dedent_count, n_to_copy);
    n_copied_so_far += n_to_copy;

    n_to_copy =
        indent_length_stack.size() * sizeof(indent_length_stack_element_type);
    ...
    std::memcpy((void *)&(buffer[n_copied_so_far]),
                (void *)indent_length_stack.data(), n_to_copy);
    n_copied_so_far += n_to_copy;
  ...
  void deserialize(const char *buffer, unsigned length) {
    ...
    std::memcpy((void *)&queued_dedent_count,
                (void *)&(buffer[n_copied_so_far]), n_to_copy);
    n_copied_so_far += n_to_copy;
    ...
    indent_length_stack.resize(n_to_copy /
                               sizeof(indent_length_stack_element_type));
  ...
  void dedent() { token_queue.push(DEDENT); }

  void newline() { token_queue.push(NEWLINE); }
...
void *tree_sitter_agda_external_scanner_create() { return new Scanner(); }
